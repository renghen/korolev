    ...
    for {
      ...
      _ <- Effect[F].delay {
        headers.collectFirst { case ("x-name", v) => v } match {
          case None => Effect[F].pure(HttpResponse(Response.Status.BadRequest, "Header 'x-name' should be defined", Nil))
          case Some(_) => app.frontend.resolveFile(descriptor, LazyBytes(chunks, body.bytesLength))
        }
      }
